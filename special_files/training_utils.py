            ...
            if shapes[i] is not None and not K.is_tensor(data[i]):
                ...
                for dim, ref_dim in zip(data_shape, shape):
                    if ref_dim != dim and ref_dim:
                        print("Be aware that " + names[i] + 
                              " doesn't have expected shape " + str(shape) + 
                              ". It has shape " + str(data_shape))
                        #raise ValueError(
                        #    'Error when checking ' + exception_prefix +
        ...
        if loss in key_losses:
            for target_dim, out_dim in zip(y.shape[1:], shape[1:]):
                if out_dim is not None and target_dim != out_dim:
                    print('Warning: target_dim and out_dim have different' +
                          ' shapes. Loss warning.')
                    #raise ValueError(
...
def standardize_weights(y,
    ...
    if sample_weight is not None:
        ...
        if len(weights) != len(y_classes):
            print('Warning: Number of classes != number of class weights.')
